...
export const cases = {
  ...
  getById: async (id) => {
    ...
    // Laravel CaseResource envuelve los datos en {data: {...}}
...
export const analytics = {
  ...
  getDashboardStats: async () => {
    return apiRequest('/analytics/dashboard-stats', {
      errorCode: 'dashboard_stats_failed'
    });
  },

  getTimeline: async (period = 30) => {
    return apiRequest(`/analytics/timeline?period=${period}`, {
      errorCode: 'timeline_failed'
    });
  },

  getCasesByType: async () => {
    return apiRequest('/analytics/cases-by-type', {
      errorCode: 'cases_by_type_failed'
    });
  }
};
